...
import com.example.mediaapprove.Login_account;
...
public class AccountRegistration extends AppCompatActivity {
    //declaring variables
    EditText mob_no,username_acc ,password_acc,rpassword_acc;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mob_no = findViewById(R.id.mobile_checked);
        username_acc = findViewById(R.id.acc_username);
        password_acc = findViewById(R.id.acc_password);
        rpassword_acc = findViewById(R.id.acc_rpassword);
        ...
        String mobilenum = sharedPreferences.getString("mobile_checked","");
        mob_no.setText(mobilenum);
        mob_no.setEnabled(false);

    }
    //onclick listener for button
    public void Reg_Account(View view) {

        //initializatiion for the params
        String getNo = mob_no.getText().toString();
        String getUser = username_acc.getText().toString();
        String getPass = password_acc.getText().toString();
        String getRPass = rpassword_acc.getText().toString();


        //checking if the ET is empty or over restriction
        if(getNo.equals("")){mob_no.setError("Provide a Mobile Number");mob_no.requestFocus();}
        else if(getUser.equals("")){username_acc.setError("Provide a Username");username_acc.requestFocus();}
        else if(getPass.equals("")) {password_acc.setError("Provide a Password");password_acc.requestFocus();}
        else if(getPass == null || getPass.length() < 6 || getPass.length() >= 60) {password_acc.setError("Please enter at least 6 character password  and do not exceed to 60 characters");password_acc.requestFocus();}
        else if(getRPass.equals("")){rpassword_acc.setError("Confirm Password is required");password_acc.requestFocus();}
        else if(!(getRPass.equals(getPass))){rpassword_acc.setError("Password Didn't match");password_acc.requestFocus();}
        else if( //checking if all the fields have value except middle name
                        !getPass.equals("")&&!getRPass.equals("")&&(getRPass.equals(getPass))){
            ...
            handler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    PutData putData = new PutData("https://mediapprove.net/android/otp/save_reg.php", "POST", field, data);
                    ...
                    if (putData.startPut()) {
                        ...
                        if (putData.onComplete()) {
                            ...
                            if(!result.equals("")){
                                ...
                                editor.putString("mobile_checked",null);
                                editor.apply();//create your session
                                //making an intent to move to another activity
                                Intent in = new Intent(getApplicationContext(), Login_account.class);
